...
We define BCSL model $\mathtt{M}$ as 4-tuple $(\mathtt{R}, \Sigma_\mathtt{A}, \Sigma_\mathtt{T}, \mathtt{init})$ where $\mathtt{R}$ is set of rules (rule expressions?), $\Sigma_\mathtt{A}$ is atomic signature, $\Sigma_\mathtt{T}$ is structure signature, and $\mathtt{init} \subseteq \mathcal{U}$ (expressions?) is a initial state (multiset).
\end{definition}
...
In the previous sections, we defined BCSL objects and the syntax. Now, we need to connect these constructs and give the model $\mathtt{M}$ its semanting meaning. For this purpose, we define semantic function $\mathtt{F}$. It is defined recursively according to expression given as argument:
...
where $\bigotimes \mathcal{G}(\mathtt{Indices})^*$ means cartesian product of sets inside of tuple $\mathcal{G}(\mathtt{Indices})$.
