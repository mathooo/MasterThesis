...
We define a multiset $O$ as a pair $(A, \mathtt{m})$ where $A$ is some set and $ \mathtt{m} : A \rightarrow \mathbb{N} $ is a function from $A$ to the set of positive integers. The set $A$ is called the \emph{underlying set of elements}. For each $a$ in $A$ the \emph{multiplicity} (that is, number of occurrences) of a is the number $\mathtt{m}(a)$.
\end{definition}
...
\label{semantic_function}
We define semantic function $\mathtt{F}$ according to the expression given in $\llbracket \ldots \rrbracket$ brackets as follows:
...
\label{ground_form}
We define ground from function $\mathcal{G}$ for objects (middle column) of particular type (left column) as:
...
In previous sections we defined all required constructs to create a chemical reaction model (Definition~\ref{crn}) from a BCSL model. In this section, we provide detailed description of this process. Particularly, we define relation between CRM and BCSL models and show how can be LTS of CRM translated to LTS of BCSL model with all necessary operations.
...
Translation of a vector $(a_1, a_2, \ldots, a_n) \in \mathbb{N}^n$ to a multiset $\mathtt{S} = (A, \mathtt{m})$ is defined as follows:
...
The procedure provided in Definition~\ref{vector_to_state} can be extended to the labels of the LTS. The labels, which represent reactions applied, might contain negative numbers. Therefore, we need to define translation function for negative numbers.
...
We define the labeled transition system of the BCSL model $\mathds{M}$, $LTS(\mathds{M}) = (S, A, T, s_0)$, from the $LTS(\mathcal{M}) = (S', A', T', s'_0)$ such that:
...
\noindent Definition of rule expressions $\varrho$:
{\footnotesize
\begin{enumerate}
...
The redundant rule $\mathtt{R}$ does not add any semantic details to the model. It generally means the LTSs produced from the models with and without the rule are equal.
...
There might be cases when eliminating context of the given BCSL model preserves some properties while making the analysis of the model simpler. To make it possible, we have to define such a reduced model and show what kind of information does it preserve.
...
\begin{center}
$\forall \mathtt{R} \in \mathcal{R}_f$ where $\mathtt{R} = (\chi, \omega, \iota, \varphi, \psi)$ holds:

$\forall \mathtt{X}' \in \chi: \neg (\mathtt{X}' \lhd \mathtt{X})$.
\end{center}
\end{theorem}
