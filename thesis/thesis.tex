...
Sets $\mathbbmss{X}_1, \ldots, \mathbbmss{X}_n \subseteq \mathds{X}$ are \emph{compatible sets} if for each compatible set $\mathbbmss{X}_i$ and $\mathbbmss{X}_j$ hold the following conditions:
\begin{enumerate}
	\item \label{compatible_cond} $ \forall \mathtt{X}, \mathtt{X}' \in \mathbbmss{X}_i: \mathtt{X} \lhd \mathtt{X}' \vee \mathtt{X}' \lhd \mathtt{X} $,
...
\begin{theorem}
In these sets, there always exists a supremum, what is an ancestor of all the remaining agents in the set. 
\end{theorem}

\begin{proof}
Proof goes here...

% via fact that we can always remove some context from complex agent, if not it is the supremum
\end{proof}
...
Lets assume a complex agent $\mathtt{X}$ belongs to two compatible sets, namely $\mathtt{X} \in \mathbbmss{X}_1, \mathbbmss{X}_2$. From the Definition~\ref{compatible_set} condition~(\ref{compatible_cond}) follows that there exists a $\mathtt{X}_1 \in \mathbbmss{X}_1$ such that either $\mathtt{X} \lhd \mathtt{X}_1$ or $\mathtt{X}_1 \lhd \mathtt{X}$ holds. 

Then, the condition~(\ref{non_compatible_cond}) claims that none of complex agents from $\mathbbmss{X}_2$ can be compatible with (in either direction) a complex agent from $\mathbbmss{X}_1$. Namely $\mathtt{X} \in \mathbbmss{X}_2$ cannot be compatible with $\mathtt{X}_1 \in \mathbbmss{X}_1$. It follows $\mathtt{X} \not\in \mathbbmss{X}_2$, what is a contradiction.
...
Before we can eliminate such rules, we have to detect them. For this purpose, compatibility operator and resulting compatible sets $\mathbbmss{X}$ can be used.
...
by "induction": show for one complex that you generate subset of complexes from "smaller" agent in compatible relation. Then, generalise it for entire rule}
\end{proof}
