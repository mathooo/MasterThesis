...
\begin{enumerate}
	...
	$$\forall i \in [1, k]: \pi_i(\widetilde{\chi}) \equiv \mathtt{sup}(\mathscr{X})$$ where $\mathscr{X}$ is a compatible set such that $\pi_i(\chi) \in \mathscr{X}$,
	...
	\begin{center}
	$ \widetilde{\mathtt{X}} \equiv \mathtt{sup}(\mathscr{X}) $ such that $\mathtt{X} \in \mathscr{X}$
	\end{center}
...
If supremum $\mathtt{sup}(\mathscr{X})$ is non-reachable in reduced model $\widetilde{\mathds{M}}$ constructed with respect to given model $\mathds{M}$, then agent $\mathtt{X}$ is also non-reachable in the LTS of the original model $\mathds{M}$.
\end{theorem}
