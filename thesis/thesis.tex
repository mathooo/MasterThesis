...
Computer simulations reproduce the behavior of a system using a mathematical model.  The numerical solution will try to mimic the behavior of the real system over time -- this is a simulation. Once we want to simulate BCSL model, regardless the approach we choose it is necessary to enrich the rules by kinetic rates. The rates basically express the speed of the rule. However, assigning a rate to the rule is not as simple as it might seem. This is discussed in Appendix~\ref{rates_discussion}.
...
\caption{A complete model Yamada 2004~\cite{yamada2004model} of FGF signaling pathway written in BCSL.}
\end{figure}
