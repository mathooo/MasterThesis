...
We define a multiset $O$ as a pair $(A, m)$ where $A$ is some set and $ m : A \rightarrow \mathbb{N} $ is a function from $A$ to the set of positive integers. The set $A$ is called the \emph{underlying set of elements}. For each $a$ in $A$ the \emph{multiplicity} (that is, number of occurrences) of a is the number $m(a)$.
\end{definition}

\begin{notation}
Let $O$ be a multiset. By $x \in O$ we usually denote the fact that there exists a pair $(x, i) \in O$ such that $i \geq 1$.
\end{notation}
...
Let $\mathcal{M} = (\mathfrak{R}, \vv{\nu}, \theta)$ be a chemical reaction model. The labeled transition system of model $\mathcal{M}$, $LTS(\mathcal{M})$ (Definition~\ref{lts}), is defined as a tuple $(S, A, T, s_0)$ such that:
...
\begin{enumerate}
	...
	\begin{tabular}{l l}
	\textit{Basis}: & $\vv{\nu} \in S$\\
	\textit{Induction}: & $ \vv{\mathtt{s}} \in S \Rightarrow \forall \vv{r} \in \mathfrak{R}: \vv{\mathtt{p}} \in S $ such that $(\vv{\mathtt{s}}, \vv{\mathtt{r}}, \vv{\mathtt{p}}) \in \varrho $\\
	\end{tabular}
 
	\item $ A = \mathfrak{R} $,
	\item $ T = \{~ (\vv{\mathtt{s}}, \vv{\mathtt{r}}, \vv{\mathtt{p}}) ~|~ \vv{\mathtt{s}}, \vv{\mathtt{p}} \in S \wedge \vv{\mathtt{r}} \in \mathfrak{R}  \wedge (\vv{\mathtt{s}}, \vv{\mathtt{r}}, \vv{\mathtt{p}}) \in \varrho ~\} $,
	\item $ s_0 = \vv{\nu} $.
\end{enumerate}
\end{definition}
...
Translation of a multiset $\mathtt{S} \subseteq \mathds{X}$ to a vector $(a_1, a_2, \ldots, a_n) \in \mathbb{N}^n$ is defined as follows:
\begin{center}
...
\begin{lemma}
In every compatible set $\mathscr{X}$ (Remark~\ref{order_in_compat_set}), there always exists a global supremum $\mathtt{sup}(\mathscr{X})$, what is an ancestor of all the remaining agents in the set.
\end{lemma}
...
Let $\mathscr{X} \subseteq \mathds{X}$ be a compatible set and $\mathtt{X} \in \mathscr{X}$ a complex agent. A set $\mkern2mu\overline{\mkern-4.5mu \mathscr{X}} \in \mathscr{X}$ is called \emph{compatible subset} of $\mathscr{X}$ w.r.t. $\mathtt{X}$ if the following holds:
...
Items of $\mkern2mu\overline{\mkern-4.5mu \mathscr{X}}$ are the minimal elements of set $\mathscr{X}$ w.r.t. complex agent $\mathtt{X}$.
...
However, when we look at the process more theoretically, a set of appropriate complex agents is generated for every complex agent $\mathtt{X}$ in the rule $\mathtt{R}$ and then they are connected to multiple unique reactions. This set is actually a compatible subset $\mkern2mu\overline{\mkern-4.5mu \mathscr{X}}$ of compatible set $\mathscr{X}$ such that $\mathtt{X} \in \mathscr{X}$. In other words, the agents in $\mkern2mu\overline{\mkern-4.5mu \mathscr{X}}$ are all minimal items from compatible set $\mathscr{X}$ which are compatible with agent $\mathtt{X}$.
...
\begin{definition}{Redundant rule}
\label{redundant_rule}

Let $\mathds{M}_1 = (\mathcal{R} \cup \{ \mathtt{R} \}, \Sigma_\mathtt{A}, \Sigma_\mathtt{T}, \mathtt{init})$ and
$\mathds{M}_2 = (\mathcal{R}, \Sigma_\mathtt{A}, \Sigma_\mathtt{T}, \mathtt{init})$ be BCSL models where $\mathtt{R}$ is a rule such that $\mathtt{R} \not\in \mathcal{R}$.

We define the rule $\mathtt{R}$ as \emph{redundant} if 

\begin{center}
$LTS(\mathds{M}_1) \equiv LTS(\mathds{M}_2)$
\end{center}
\end{definition}
...
A reduced model $\widetilde{\mathds{M}}$ is actually an over-approximation of a BCSL model $\mathds{M}$ in context of their LTSs (Definition~\ref{over_approximation}). It can be used for some types of analyses which avoid combinatorial explosion of the original model.

\begin{definition}{Over-approximation of LTS}
\label{over_approximation}

Let $LTS, LTS'$ be labeled transition systems of some BCSL models. The $LTS'$ is \emph{over-approximation} of $LTS$ if for every sequence of states $S'$ such that there exists transition between them in $LTS'$ there exists a sequence of states $S$ such that there exists transition between them in $LTS$ such that:

\begin{center}
$\forall \mathtt{s'} \in S' \exists \mathtt{s} \in S : \forall \mathtt{x'} \in \mathtt{s'} \exists \mathtt{x} \in \mathtt{s} : \mathtt{x'} = \mathtt{sup}(\mathtt{x}) $.
\end{center}
