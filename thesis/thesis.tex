...
Atomic agent $\mathtt{A}$ is a pair $(\mu, \delta)$ where $\mu \in \mathcal{N}_{A}$ is a name and $\delta~\in~\mathcal{N}_{\delta} \cup \{ \varepsilon \}$ is a state.
\end{definition}

\begin{definition}{Equivalence of atomic agents}
Let $\mathtt{A}, \mathtt{A}'$ be atomic agents. $\mathtt{A}$ is \emph{equivalent} with $\mathtt{A}'$, written $\mathtt{A} \equiv \mathtt{A}'$, iff $\mu(\mathtt{A}) \equiv \mu(\mathtt{A}') \wedge \delta(\mathtt{A}) \equiv \delta(\mathtt{A}')$.
\end{definition}
...
Structure agent $\mathtt{T}$ is a pair $(\mu, \gamma)$ where $\mu \in \mathcal{N}_{T}$ is a name and $\gamma \subseteq \mathcal{A}$ is a set of atomic agents called partial composition.
\end{definition}
...
\begin{center}
$\bullet_{\tau \in T}^n (\tau) = (x_1^1, \ldots, x_m^1, x_1^2, \ldots, x_l^2, \ldots \ldots, x_1^n, \ldots, x_k^n)$.
\end{center}
\end{definition}
...
In the previous sections, we defined BCSL objects and the syntax. Now, we need to connect these constructs and give the model $\mathtt{M}$ its semanting meaning. For this purpose, we define semantic function $\mathtt{F}$. It is defined recursively according to context given as argument:
...
\begin{definition}{Rule syntax definition}\label{rules:directions}
...
\begin{definition}{Stoichiometry}\label{rules:stoichiometry}
...
\begin{definition}{Locations}\label{rules:locations}
...
\begin{definition}{variables}\label{rules:variables}
