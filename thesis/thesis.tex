...
We define a rule $\mathtt{R}$ as \emph{redundant rule} if [something about LTSs comparison goes here]
...
The rule $\mathtt{R}$ is \emph{redundant} if
...
A reduced model $\widetilde{\mathds{M}}$ is actually an over-approximation of an BCSL model $\mathds{M}$. It can be used for some types of analyses which avoid combinatorial explosion of the original model.
