...
\begin{center}
$\mathtt{reassemble}(\mathcal{G}(\mathtt{R})) = \{~  y ~|~ x \in \mathcal{G}(\mathtt{R}) \wedge y = \Xi_{i=1}^{|x|} x_i ~\}$
\end{center}

\noindent\rule{\textwidth}{2pt}

\section{Constructing reactions}
...
\begin{definition}{Reactions constructing}\label{generate_reactions}
\hspace*{-0.8cm}\begin{tabular}{ r l }
$\mathtt{Reactions}(\mathtt{R}) =$ & $\{~ \mathtt{r}(\mathtt{seq}, \mathtt{I}) ~|~ y \in \mathtt{reassemble}(\mathcal{G}(\mathtt{R})) ~\}$,\\
...
Vectorized model $\mathcal{M}$ is a triple $(\vv{\mathds{R}}, \vv{\nu}, \pi)$ such that $\vv{\mathds{R}} \subseteq \mathbb{Z}^n$ is set of vector reactions, $\vv{\nu} \in \mathbb{N}_0^n$ is initial vector state, and $\pi \in \mathcal{X}^n$ is vector of reference complexes for some $n \in \mathbb{N}$.
\end{definition}
...
Reference vector $\pi$ is a tuple of all possible unique complexes constructed from $\mathtt{Reactions}$ as following:
...
\item create initial vector state $\vv{\nu}$ (Definition~\ref{state_to_vector}),
\item construct set of vector reactions $\vv{\mathcal{R}}$ from set of $\mathtt{r} \in \mathtt{Reactions}(\mathtt{R})$, where individual vector reaction $\vv{r}$ is created from $\mathtt{r}$ as following:
