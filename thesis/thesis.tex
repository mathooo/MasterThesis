...
\begin{center}
$\forall s_i, s_{i+1} : (s_i, a, s_{i+1}) \in T$ for some $a \in A$.
\end{center}
...
Note that procedure provided in Definition~\ref{vector_to_state} can be extended to the labels of the LTS. The labels, which represent reactions applied, might contain negative numbers. These are treated independently and represent left-hand-side of the reaction $\lambda^{T-}$ (the reactants) while non-negative numbers represent right-hand-side of the reaction $\lambda^{T+}$ (the products).
...
We define the labeled transition system of BCSL model $\mathds{M}$, $LTS(\mathds{M}) = (S, A, T, s_0)$, from the $LTS(\mathcal{M}) = (S', A', T', s'_0)$ as follows:

\begin{enumerate}
	\item $S = \{~ s ~|~ s = \lambda^T(s', \theta) \wedge s' \in S' ~\}$,
	\item $A = \{~ (l, r) ~|~ l = \lambda^{T-}(a', \theta) \wedge r = \lambda^{T+}(a', \theta) \wedge a' \in A' ~\} $,
	\item $T = \{~ (s, (l, r), t) ~|~ s = \lambda^T(\vv{\mathtt{s}}, \theta) \wedge t = \lambda^T(\vv{\mathtt{p}}, \theta) \wedge l = \lambda^{T-}(\vv{\mathtt{r}}, \theta) \wedge r = \lambda^{T+}(\vv{\mathtt{r}}, \theta) \wedge (\vv{\mathtt{s}}, \vv{\mathtt{r}}, \vv{\mathtt{p}}) \in T' ~\} $,
	\item $s_0 = \lambda^T(s'_0, \theta)$.
\end{enumerate}
...
define compatibility of states of LTS
\end{definition}

\begin{definition}{Over-approximation of LTS}
\label{over_approximation}

Let $LTS, LTS'$ be labeled transition systems of some BCSL models. The $LTS'$ is \emph{over-approximation} of $LTS$ if for every path of states $s_1' s_2' s_3' \ldots s_n'$ in $LTS'$ there exists a path of states $s_1 s_2 s_3 \ldots s_m$ in LTS such that:

\begin{center}
$\forall s'_i, s'_{i+1} \exists s_k, s_l : l > k \wedge  $
