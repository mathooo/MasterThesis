...
$(\chi, \omega, \mathtt{I}, \mathtt{map}, \mathtt{Indices}) \in \mathds{R}$ :\\
\end{tabular}
\end{center}
...
and $\mathtt{R} = (\chi, \omega, \mathtt{I}, \mathtt{map}, \mathtt{Indices})$. Moreover, $\mathtt{A} = (\eta, \delta)$, $\mathtt{A}' = (\eta', \delta')$, 

$\mathtt{T} = (\eta, \gamma)$, and $\mathtt{T}' = (\eta', \gamma')$.
}
\end{definition}

bla bla continues here
...
& ~~$\mathtt{X}(y(\pi_{|\mathtt{map}| - 1}(\mathtt{map}), \ldots, \pi_{|\mathtt{map}|}(\mathtt{map}) - 1), \mathtt{com}(\pi_{|\mathtt{map}|}(\chi)))\big) $\\
 ...
 & ~~and $ \mathtt{I} \in \mathtt{R}$, $\chi \in \mathtt{R}$, $\mathtt{map} \in \mathtt{R}.$\\
\end{tabular}
\end{definition}
