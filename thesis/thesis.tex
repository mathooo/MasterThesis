...
Let $\mathtt{X}, \mathtt{X}'$ be complexes. $\mathtt{X}$ is \emph{equivalent} to $\mathtt{X}'$, written $\mathtt{X} \equiv \mathtt{X}'$, iff $\mathtt{len}(\mathtt{X}) \equiv \mathtt{len}(\mathtt{X'}) \wedge \exists \mu' \in \sigma(\mu(\mathtt{X}'))$ such that $\forall i~\in~[1,~n]: \pi_i(\mu(\mathtt{X})) \equiv \pi_i(\mu')$.
\end{definition}
...
Let $\mathtt{R}$ be a rule and $i,j \in \mathbb{N}$ natural numbers. We say the rule $\mathtt{R}$ is \emph{well-formed} if the following conditions hold:
...
\begin{notation}
We denote left-hand-side of the reaction $(\pi_0(\mathtt{seq}), \ldots, \pi_{\mathtt{I}-1}(\mathtt{seq}))$ as $LHS(\mathtt{r})$ and right-hand-side of the reaction $(\pi_\mathtt{I}(\mathtt{seq}), \ldots, \pi_n(\mathtt{seq}))$ as $RHS(\mathtt{r})$.
\end{notation}
...
First of all, we need to define tuples concatenation, since we work with them quite often want to make sure this operation is clear.
...
Let $T = (T_1, T_2, \ldots, T_n)$ be sequence of tuples. Concatenation of sequence of tuples $\Xi_{i=1}^n T_i$ is defined as:
...
Next, we need to define projection $\pi$ to enable iterating through multidimensional structures.

\begin{definition}{Projection}
\begin{center}
$ \pi_i = ((x_1, \ldots, x_n) \mapsto x_i) : X^n \rightarrow X $
\end{center}
\end{definition}

Reassembly operation is again quite simple action on tuples. Intuitively, we need to reorganise tuple which has form $((a_1, b_1), (a_2, b_2),\\ \ldots, (a_n, b_n))$ to a tuple of form $(a_1, a_2, \ldots, a_n, b_1, b_2, \ldots,  b_n)$. We define reassembly generally on a set of tuples. 
...
\begin{center}
$\mathtt{reassembly}(Y) = \{~  x ~|~ y \in Y \wedge x = \Xi_{i=1}^{|y|} \pi_1(y_i) \mdoubleplus \Xi_{i=1}^{|y|} \pi_2(y_i) ~\}$
\end{center}
\end{definition}
...
where $\mathcal{G}(\mathtt{Indices})$ is an ordered set according to order function $f: \mathtt{Indices} \rightarrow \mathbb{N}$. This function assigns to each item $(i,j) \in \mathtt{Indices}$ a unique position $f(i,j) = k$. An item $\mathcal{G}(\pi_i(\omega), \pi_j(\omega)) \in \mathcal{G}(\mathtt{Indices})$ preserves this position $k$.
...
\begin{center}
$(\vv{\upsilon}, \vv{\mathtt{r}}, \vv{\mathtt{u}}) \in \varrho$ $\Leftrightarrow$ $\vv{\mathtt{u}} = \vv{\upsilon} + \vv{\mathtt{r}} \wedge N(\vv{\mathtt{u}})$.
\end{center}
\end{definition}
...
\noindent Then, reference vector $\theta \in \Theta^n$ such that $\forall i,j \leq n : \Theta_i \not\equiv \Theta_j$ and $n~=~|\Theta|$. Note there are $n!$ possibilities how to choose $\theta$, arbitrary can be chosen.
...
\begin{proof}
The theorem follows from Definition~\ref{compatible_set} condition~(\ref{compatible_cond}) which basically claims that there is a supremum (in manner of compatibility) for each two complex agents in the compatible set $\mathbbmss{X}$. Since there exists a supremum for every two items of the set and the set $\mathbbmss{X}$ is finite (condition~\ref{finite_cond}), there must exists a global supremum for the entire set.
\end{proof}
...
$$ \mathtt{X}_1 + \ldots + \mathtt{X}_n \Rightarrow \mathtt{X}_{n+1} + \ldots + \mathtt{X}_{m} $$
...
$$ \forall i \in [ 1, m ]: \mathtt{X}_i \lhd \mathtt{X}'_i. $$
\end{theorem}
...
For the proof we can use compatible sets $\mathbbmss{X}$ and the fact that when we are generating reactions from rules, we are actually enumerating all agents from the set $\mathbbmss{X}$ which are \emph{compatible with} original agent in the rule (Remark~\ref{generating_vs_compatibility}).
...
The complex agents $\mathtt{X}_k, \mathtt{X}'_k$ both belong to the same compatible set $\mathbbmss{X}$ since $\mathtt{X}_k \lhd \mathtt{X}'_k$, which follows from the condition of the theorem. 
...
Therefore, the produced set of reactions from the redundant rule is actually a subset of reactions produced from the non-redundant rule. The redundant rule does not bring any semantical acquisition.
\end{proof}

\subsection{Context-based reduction}
\label{context_reduction}

Context reduced model (CRM) is created form given BCSL model by reducing context of complexes in rules to maximum level. This is achieved by taking supremum from compatible set $\mathbbmss{X}$. This procedure might produce rules with no action, i.e. not well-formed rules -- such rules might be omitted. Then, only rules creating/destroying agents and complex formation/dissociation should remain, providing reduced network. Since we are reducing context, the resulting network can be equal or smaller than the original one.

\begin{definition}{Context reduced model}
formally define + define formally how the model is constructed from the original model!
\end{definition}

Moreover, the resulting network looses its rule-based properties and becomes a flat structure -- a reaction-based network.

If we are checking whether an agent is reachable for given model, we might first check whether it's super-ancestor is reachable in created CRN. If so, then we are still not certain about reachability of agent itself and it has to be checked in the original model. However, when it comes to non-reachability, agent which is non-reachable in CRN is not reachable in the original model too.

\begin{theorem}
We want to check whether an complex agent $\mathtt{X}$ is reachable in the LTS of a given BCSL model $\mathds{M}$. Let $\mathbbmss{X}$ be a compatible set for $\mathtt{X}$.

If supremum $\mathtt{sup}(\mathbbmss{X})$ is non-reachable in context reduced model CRM of given model $\mathds{M}$, then agent $\mathtt{X}$ is also non-reachable in the LTS of the original model $\mathds{M}$.
\end{theorem}

\begin{proof}
by controversy ? we will assume an agent is non reach in CRM but reach in $\mathds{M}$
\end{proof}
