...
Before we proceed, we need to define projection $\pi_i$ to enable iterating through multidimensional structures.

\begin{definition}{Projection}
Projection $\pi_i : X^n \rightarrow X $ is defined as follows:

\begin{center}
$ \pi_i = ((x_1, \ldots, x_n) \mapsto x_i) $
\end{center}
\end{definition}

We also need to define permutations...
...
A partition $\mathcal{P} = B_1/B_2/\ldots/B_k$ of $[1,\ldots,n]$ is non-crossing if  whenever a quadruple of elements $1 \leq a < b < c < d \leq n$ satisfies $a, c \in B_i$ and $b, d \in B_j$ for some $1 \leq i, j \leq k$, then in fact $i = j$; thus, the blocks do not cross.
\end{definition}
...
\begin{center}
\begin{tabular}{ c c l }
Object type & Arguments & Ground form \\
\hline
 & & \\
