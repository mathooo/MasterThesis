...
Let $\mathscr{X}$ be a compatible set and $\mathtt{X}$ a complex agent such that $\mathtt{X} \in \mathscr{X}$. A set $\mathscr{X}_\subseteq$ is a \emph{compatible subset} w.r.t. $\mathtt{X}$ \emph{iff}:
