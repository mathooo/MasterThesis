...
\end{minipage}
\caption{\textbf{Example of a map-apply action}. As a solution, we use the solution (a) from Fig.~\ref{solutions:fig}. \textbf{(a)} Rule can be mapped on a \raisebox{-0.02cm}[0cm]{{\large $\square$}} and a $\bigcirc$ regardless their colors and form a complex \raisebox{-0.02cm}[0cm]{{\large $\square$}}--$\bigcirc$.  \textbf{(b)} We randomly choose the yellow \raisebox{-0.02cm}[0cm]{{\large $\square$}} and the green $\bigcirc$ from our solution. The rule was mapped on chosen objects and they were assigned to the left-hand side of the rule. \textbf{(c)} The rule was applied and new object (the yellow-green complex \raisebox{-0.02cm}[0cm]{{\large $\square$}}--$\bigcirc$) was created. We obtained the reaction describing the particular action which has just happened.}
\label{map-apply:fig}
\end{center}
\end{figure}
...
where all descending intervals in definition of $\psi$ are ignored and $\psi$ is lexicographically sorted set such that symbol $'-' > i$ for every $i \in \mathbb{N}$.
...
\label{ground_form}
We define \emph{ground from function} $\mathcal{G}$ for objects (middle column) of particular type (left column) as:
...
Lets have two rule expressions:
