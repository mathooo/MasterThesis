...
In our case, reachability analysis can be generalised to checking whether for some multiset of complex agents $\mathtt{S}$ there exists a state $s \in S$ of an LTS $\mathcal{L}$ such that $\mathtt{S} \subseteq s$. In other words, we are interested only in particular agents regardless other context in the reachable rule. This might generally lead to multiple reachable states and then we can for example consider a shortest path etc.

\subsection{Simulation}
\label{simulation}

Once we want to simulate BCSL model, regardless the approach we choose it is necessary to enrich the rule by kinetic rates. The rates basically express the speed of the rule. However, assigning a rate to the rule is not as simple as it might seem. This is discussed in Appendix~\ref{rates_discussion}.
