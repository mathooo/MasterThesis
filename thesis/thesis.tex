...
\newenvironment{runningExample}{\bigskip\refstepcounter{counter}\noindent\textbf{Running example \thecounter}\par\nopagebreak}{\bigskip}
...
\begin{definition}{Equivalence of complex agents}
...
\begin{center}
$\mathtt{reassemble}(\mathcal{G}(\mathtt{R})) = \{~  \tau ~|~ \tau = \Xi_{i=1}^{|\mathcal{G}(\mathtt{R})|} \mathcal{G}(\mathtt{R})_i ~\}$
\end{center}
...
\begin{tabular}{ ll }
$\mathtt{Reactions}(\mathtt{R}) =$ & $\{~ \mathtt{r}(\mathtt{seq}, \mathtt{I}) ~|~ T \in \mathtt{reassemble}(\mathcal{G}(\mathtt{R})) ~\wedge$\\

& $seq = (\mathtt{X}(T(0, \ldots, \mathtt{map}(\mathtt{R})[0] - 1), \mathtt{com}(\chi(0))),$ \\

& $\mathtt{X}(T(\mathtt{map}(\mathtt{R})[0], \ldots, \mathtt{map}(\mathtt{R})[1] - 1), \mathtt{com}(\chi(1))), \ldots,$\\

& $\mathtt{X}(T(\mathtt{map}(\mathtt{R})[|\mathtt{map}(\mathtt{R})| - 1], \ldots, \mathtt{map}(\mathtt{R})[|\mathtt{map}(\mathtt{R})|]), \mathtt{com}(\chi(|\mathtt{map}(\mathtt{R})|)))) ~\wedge$\\

& $ \mathtt{I} \in \mathtt{R} \wedge \chi \in \mathtt{R} \wedge \mathtt{map}(\mathtt{R}) = (i_1, \ldots, i_n) ~\}$
\end{tabular}
...
For better demonstration, we give running example, which will go through all syntactic extensions.
...
\begin{runningExample}\label{run1.1}
...
We define neccesary condition, when this notation is allowed.

\begin{definition}{conditions}\label{locations:conditions}
