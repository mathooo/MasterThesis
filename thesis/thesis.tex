...
\begin{enumerate}
	...
	where $\mathscr{X}$ is a compatible set such that $\pi_i(\chi) \in \mathscr{X}$ and length $k = |\chi'| = |\chi|$ (i.e., number of complex agent expressions is the same in both sides of the rules respectively),
...
Let $LTS, LTS'$ be labeled transition systems of some BCSL models. The $LTS'$ is \emph{over-approximation} of $LTS$ if for every path of states $S'$ in $LTS'$ there exists a path of states $S$ in LTS such that:
...
Let $\mathtt{X}$ be a complex agent, $\mathscr{X}$ be a compatible set for $\mathtt{X}$, and $\mathds{M}$ be a given BCSL model. If supremum $\mathtt{sup}(\mathscr{X})$ is non-reachable in LTS of reduced model $\widetilde{\mathds{M}}$ constructed with respect to given model $\mathds{M}$, then agent $\mathtt{X}$ is also non-reachable in the LTS of the original model $\mathds{M}$.
\end{theorem}
