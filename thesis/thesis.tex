...
supporting me over the years. You have set an example of excellence as a researcher, mentor, and instructor.

\bigskip

\noindent  I would like to thank my colleges of Systems Biology Laboratory for all discussions, help, and feedback the provided.  
...
\label{BCSL_model}
BCSL model $\mathds{M}$ is a quadruple $(\mathcal{R}, \Sigma_\mathtt{A}, \Sigma_\mathtt{T}, \mathtt{init})$ where $\mathcal{R}$ is set of rule expressions, $\Sigma_\mathtt{A}$ is atomic signature, $\Sigma_\mathtt{T}$ is structure signature, and $\mathtt{init}$ is initial multiset of complex expressions.
\end{definition}
...
\begin{enumerate}
	...
	\begin{tabular}{l l}
	\textit{Basis}: & $\vv{\nu} \in S$\\
	\textit{Induction}: & $ \vv{\mathtt{s}} \in S \Rightarrow \forall \vv{r} \in \mathfrak{R}: \vv{\mathtt{p}} \in S $ such that $(\vv{\mathtt{s}}, \vv{\mathtt{r}}, \vv{\mathtt{p}}) \in \zeta $\\
	\end{tabular}
...
Translation of the negative part a vector is defined almost equally than the translation in Definition~\ref{vector_to_state} with exception that the condition $a_i > 0$ is modified to $a_i < 0$.
\end{definition}
